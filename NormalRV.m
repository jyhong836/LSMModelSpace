function [testAcc valAcc] = NormalRV(datasetsname,lambdas,R_no,nForgetPoints,costs,kps,leaveout, dt_out)
% test  reservior kernel with normal state distribution using svm
...
for k=1:kFold 
    ...
    for i = 1:nLambdas
        ...
        val = lambdas(i);
        [trX,teX] = compute_KernelRV_normal(trXD,teXD,R_no,val,nForgetPoints, dt_out, SpikingInput);
        %%
        num1 = size(trX,1);
...
[trainX,testX] = compute_KernelRV_normal(training,testing,R_no,lambda,nForgetPoints, dt_out, SpikingInput);
